...
#include	"..\common\common.h"	// for UARTprintf
#endif
...
void DrawCamName(BYTE dmode, BYTE resol)
{
	...
	BYTE i, pSTR[32], len=strlen(CH01);
	WORD x, y;
	...
	switch(dmode)
	{
		case MDIN_4CHVIEW_ALL :
			x = 896; y = 524;
		break;
		case MDIN_4CHVIEW_CH01 :
		case MDIN_4CHVIEW_CH02 :
		case MDIN_4CHVIEW_CH03 :
		case MDIN_4CHVIEW_CH04 :
			x = 16; y = 16;
		break;
		case MDIN_4CHVIEW_CH12_V :	
		case MDIN_4CHVIEW_CH34_V :
			y = 524;
			if((resol == NCRESOL_1280X480) || (resol == NCRESOL_1280X576))		x = 256;
			else if((resol == NCRESOL_1440X480) || (resol == NCRESOL_1440X576))	x = 184; 
		break;	
	...
	stOSD[SPRITE_INDEX8].x				= x;
	stOSD[SPRITE_INDEX8].y				= y;
	stOSD[SPRITE_INDEX8].stBMP.w		= 12*32;			// stFONT.w * 32-column
	if((dmode == MDIN_4CHVIEW_ALL) ||(dmode == MDIN_4CHVIEW_CH12_V) ||(dmode == MDIN_4CHVIEW_CH34_V))
		stOSD[SPRITE_INDEX8].stBMP.h		= 16*2;				// stFONT.h * 2-line
	else
		stOSD[SPRITE_INDEX8].stBMP.h		= 16*1;				// stFONT.h * 2-line
	...
	// set GAC charater mode with sprite8
	...
	// display font (background: blue, font: white)
	// clear remainder of font
	for (i=0; i< 32; i++) pSTR[i] = 0x00;	// blank font & transparency attb
	MDINGAC_SetDrawCHMode(0, 0, pSTR, 32, 0x00);
	MDINGAC_SetDrawCHMode(1, 0, pSTR, 32, 0x00);

	if(dmode == MDIN_4CHVIEW_ALL)
	{
		for (i=0; i<len; i++) pSTR[i] = CH01[i]-0x20;	// ascii offset
		MDINGAC_SetDrawCHMode(0, 0, pSTR, len, 0x71);

		for (i=0; i<len; i++) pSTR[i] = CH02[i]-0x20;	// ascii offset
		MDINGAC_SetDrawCHMode(0, 6, pSTR, len, 0x71);

		for (i=0; i<len; i++) pSTR[i] = CH03[i]-0x20;	// ascii offset
		MDINGAC_SetDrawCHMode(1, 0, pSTR, len, 0x71);

		for (i=0; i<len; i++) pSTR[i] = CH04[i]-0x20;	// ascii offset
		MDINGAC_SetDrawCHMode(1, 6, pSTR, len, 0x71);
	}
	else if(dmode == MDIN_4CHVIEW_CH12)
	{
		for (i=0; i<len; i++) pSTR[i] = CH01[i]-0x20;	// ascii offset
		MDINGAC_SetDrawCHMode(0, 0, pSTR, len, 0x71);
	...
	else if(dmode == MDIN_4CHVIEW_CH34)
	{
		for (i=0; i<len; i++) pSTR[i] = CH03[i]-0x20;	// ascii offset
		MDINGAC_SetDrawCHMode(0, 0, pSTR, len, 0x71);
	...
	else if(dmode == MDIN_4CHVIEW_CH12_V)
	{
		for (i=0; i<len; i++) pSTR[i] = CH01[i]-0x20;	// ascii offset
		MDINGAC_SetDrawCHMode(0, 0, pSTR, len, 0x71);
	...
	else if(dmode == MDIN_4CHVIEW_CH34_V)
	{
		for (i=0; i<len; i++) pSTR[i] = CH03[i]-0x20;	// ascii offset
		MDINGAC_SetDrawCHMode(0, 0, pSTR, len, 0x71);
	...
	else if(dmode == MDIN_4CHVIEW_CH01)
	{
		for (i=0; i<len; i++) pSTR[i] = CH01[i]-0x20;	// ascii offset
		MDINGAC_SetDrawCHMode(0, 0, pSTR, len, 0x71);
	}
	else if(dmode == MDIN_4CHVIEW_CH02)
	{
		for (i=0; i<len; i++) pSTR[i] = CH02[i]-0x20;	// ascii offset
		MDINGAC_SetDrawCHMode(0, 0, pSTR, len, 0x71);
	}
	else if(dmode == MDIN_4CHVIEW_CH03)
	{
		for (i=0; i<len; i++) pSTR[i] = CH03[i]-0x20;	// ascii offset
		MDINGAC_SetDrawCHMode(0, 0, pSTR, len, 0x71);
	}
	else if(dmode == MDIN_4CHVIEW_CH04)
	{
		for (i=0; i<len; i++) pSTR[i] = CH04[i]-0x20;	// ascii offset
		MDINGAC_SetDrawCHMode(0, 0, pSTR, len, 0x71);
	}
